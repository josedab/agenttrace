...
func TestStreamingService_LiveMetrics(t *testing.T) {
	...
	t.Run("returns nil for unknown trace", func(t *testing.T) {
		metrics := svc.GetLiveMetrics(uuid.New(), projectID)
	...
	t.Run("updates metrics on observation events", func(t *testing.T) {
		...
		metrics := svc.GetLiveMetrics(traceID, projectID)
	...
	t.Run("tracks file and terminal events", func(t *testing.T) {
		svc.PublishActivity(ctx, domain.StreamActivity{
			ID: uuid.New().String(), TraceID: traceID, ProjectID: projectID,
...
func TestStreamingService_RecentActivities(t *testing.T) {
	...
	t.Run("returns limited activities", func(t *testing.T) {
		activities := svc.GetRecentActivities(traceID, projectID, 5)
		assert.Len(t, activities, 5)
	})

	t.Run("returns all if limit exceeds count", func(t *testing.T) {
		activities := svc.GetRecentActivities(traceID, projectID, 100)
		assert.Len(t, activities, 10)
	})

	t.Run("returns nil for unknown trace", func(t *testing.T) {
		activities := svc.GetRecentActivities(uuid.New(), projectID, 10)
...
func TestStreamingService_Intervention(t *testing.T) {
	...
	t.Run("creates and retrieves intervention", func(t *testing.T) {
		...
		pending := svc.GetPendingInterventions(traceID, projectID)
	...
	t.Run("acknowledges intervention", func(t *testing.T) {
		...
		err := svc.AcknowledgeIntervention(traceID, projectID, pending[0].ID)
		require.NoError(t, err)
...
func TestStreamingService_IsolatesDuplicateTraceIDsByProject(t *testing.T) {
	realtime := NewRealtimeService()
	svc := NewStreamingService(zap.NewNop(), realtime)
	traceID := uuid.New()
	projectOne := uuid.New()
	...
	streamOne := svc.GetOrCreateStream(traceID, projectOne)
	streamTwo := svc.GetOrCreateStream(traceID, projectTwo)

	require.NotSame(t, streamOne, streamTwo)
	assert.Equal(t, projectOne, streamOne.ProjectID)
	assert.Equal(t, projectTwo, streamTwo.ProjectID)
}
...
func TestStreamingService_CleanupStream(t *testing.T) {
	...
	svc.GetOrCreateStream(traceID, projectID)
	assert.NotNil(t, svc.GetLiveMetrics(traceID, projectID))

	svc.CleanupStream(traceID, projectID)
	assert.Nil(t, svc.GetLiveMetrics(traceID, projectID))
}
