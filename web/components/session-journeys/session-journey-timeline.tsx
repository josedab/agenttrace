...
import { Search, Clock, DollarSign, Layers, Brain, Route } from 'lucide-react';
...
import { useSessionJourney } from '@/hooks/use-session-journeys';
...
export function SessionJourneyTimeline({
  ...
  const { data: journey, isLoading } = useSessionJourney(submittedId || null);
            ...
            <CardContent>
              <div className="flex h-10 overflow-hidden rounded-lg">
                {phases.map((phase) => {
                  ...
                  const widthPercent =
                    totalDuration > 0 ? (phase.durationSeconds / totalDuration) * 100 : 0;
                  return (
                    <div
                      key={phase.id}
                      className={cn(
                ...
                <Card key={phase.id}>
                  <CardContent className="flex items-center justify-between p-4">
                    <div className="flex min-w-0 items-center gap-3">
                      <div className={cn('h-10 w-1.5 shrink-0 rounded-full', config.color)} />
                    ...
                    <div className="flex shrink-0 items-center gap-4 text-sm text-muted-foreground">
                      ...
                      <span className="text-xs">{phase.tokenCount.toLocaleString()} tokens</span>
                    </div>
                  </CardContent>
                </Card>
              );
