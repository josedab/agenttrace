...
describe("usePrompts", () => {
  ...
  it("fetches all prompts with default filters", async () => {
    const mockPrompts = [
      { name: "greeting", versions: 3 },
      { name: "summary", versions: 1 },
    ];
...
describe("usePrompt", () => {
  ...
  it("fetches a single prompt by name", async () => {
    const mockPrompt = { name: "greeting", prompt: "Hello {{name}}", versions: 2 };
...
describe("useCreatePrompt", () => {
  ...
  it("creates a prompt", async () => {
    mockedApi.prompts.create.mockResolvedValue({ name: "new-prompt" });
...
describe("useUpdatePrompt", () => {
  ...
  it("creates a new version of a prompt", async () => {
    mockedApi.prompts.createVersion.mockResolvedValue({ version: 2 });
...
describe("useCompilePrompt", () => {
  ...
  it("compiles a prompt with variables", async () => {
    mockedApi.prompts.compile.mockResolvedValue({ compiled: "Hello Alice!" });
    ...
    expect(mockedApi.prompts.compile).toHaveBeenCalledWith(
      "greeting",
      1,
      { name: "Alice" }
    );
